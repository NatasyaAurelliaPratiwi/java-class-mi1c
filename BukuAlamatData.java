/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package natasya160426;

/**
 *
 * @author LAB-SI-PC
 */
public class DataBukuAlamat {
    BukuAlamat[] data = new BukuAlamat[100];
    
    public void insert(BukuAlamat bukuAlamat, int index){
        data[index] = bukuAlamat;
    }
    
    public void update(BukuAlamat bukuAlamat, int index){
        data[index] = bukuAlamat;
    }
    public void delete(int index){
        data[index] =  null;
    }
    public void View(){
        for(int i=0;i<data.length;i++){
            if (data[i] != null){
            System.out.println("Data ke-->"+ (i+1));
            System.out.println("Nama            :"+data[i].getNama());
            System.out.println("Alamat          :"+data[i].getAlamat());
            System.out.println("NomorTelepon    :"+data[i].getNomorTelepon());
            System.out.println("AlamatEmail     :"+data[i].getAlamatEmail());
            System.out.println("-----------------------------------------------");
            }
        }
    }
}
